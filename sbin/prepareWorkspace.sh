...
updateOpenj9Sources() {
  ...
  if [ "${BUILD_CONFIG[BUILD_VARIANT]}" == "${BUILD_VARIANT_OPENJ9}" ]; then
    ...
    bash get_source.sh --openssl-version=openssl-3.0.13+CVEs3 --openssl-repo=https://github.com/ibmruntimes/openssl.git ${OPENJCEPLUS_FLAGS} ${GSKIT_FLAGS} ${GSKIT_CREDENTIALS}
    cd "${BUILD_CONFIG[WORKSPACE_DIR]}"
  fi
}
