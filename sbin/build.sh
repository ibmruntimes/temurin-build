...
configureVersionStringParameter() {
  ...
  if [[ "${BUILD_CONFIG[BUILD_VARIANT]}" == "${BUILD_VARIANT_TEMURIN}" ]]; then
    ...
    # (e.g. jdk-11.0.32.10+1 instead of jdk-11.0.32.1+1 for .10 CPU releases).
...
getJdkArchivePath() {
  ...
  # For Semeru/OpenJ9 non-JDK8 builds: use the IBM vendor version (IMPLEMENTOR_VERSION)
  # as the archive directory name so the extracted directory matches the IBM version
  # scheme (e.g. "jdk-11.0.32.10" instead of "jdk-11.0.32.1+1").
  if [[ "${BUILD_CONFIG[BUILD_VARIANT]}" == "${BUILD_VARIANT_OPENJ9}" ]] \
    && [[ "${BUILD_CONFIG[OPENJDK_FEATURE_NUMBER]}" != "8" ]] \
